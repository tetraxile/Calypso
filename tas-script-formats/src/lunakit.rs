...
pub fn parse_lunakit(data: &[u8]) -> Result<Vec<internal::Frame>> {
    let (header, rest) = Header::try_ref_from_prefix(data).map_err(|_| eyre!("failed to parse script header"))?;
    ...
    if header.is_two_player {
        Ok(frames
            .chunks_exact(2)
            .map(|chunk| {
                let part_1 = &chunk[0];
                let part_2 = &chunk[1];
                ensure!(part_1.step.get() == part_2.step.get(), "invalid two-player script");
                Ok(internal::Frame {
